    ...
    <main class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
      <div class="px-4 py-6 sm:px-0">
        <div class="flex justify-between items-center mb-8">
          <div>
            <h1 class="text-3xl font-bold text-gray-900">MCP API Keys</h1>
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          <div class="flex">
            ...
            <div class="ml-3 flex-1">
              <h3 class="text-sm font-medium text-blue-800">Connect to MCP Server</h3>
              <p class="text-sm text-blue-700 mt-1">
                Add this to your <code class="bg-blue-100 px-1 rounded">.mcp.json</code> configuration:
              </p>
              <pre class="mt-2 bg-blue-100 rounded p-3 text-xs overflow-x-auto text-blue-900">{
  "mcpServers": {
    "trinity": {
      "url": "{{ mcpServerUrl }}",
      "headers": {
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
          ...
          <div v-else-if="apiKeys.length === 0" class="text-center py-12">
            ...
            <p class="mt-1 text-sm text-gray-500">Create an API key to start using the MCP server.</p>
          </div>

          <ul v-else class="divide-y divide-gray-200">
            <li v-for="key in apiKeys" :key="key.id" class="p-6 hover:bg-gray-50">
              <div class="flex items-center justify-between">
                <div class="flex items-center flex-1">
                  <div class="flex-shrink-0">
                    <div class="h-10 w-10 rounded-full flex items-center justify-center"
                         :class="key.is_active ? 'bg-green-100' : 'bg-red-100'">
                      <svg class="h-6 w-6" :class="key.is_active ? 'text-green-600' : 'text-red-600'" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
                      </svg>
                    </div>
                  </div>
                  <div class="ml-4 flex-1">
                    ...
                    <p v-if="key.description" class="mt-1 text-sm text-gray-500">{{ key.description }}</p>
                    <div class="mt-1 flex items-center text-xs text-gray-400 space-x-4">
                      <span>
                        <code class="bg-gray-100 px-1.5 py-0.5 rounded font-mono">{{ key.key_prefix }}...</code>
                      </span>
                ...
                <div class="ml-4 flex items-center space-x-2">
                  <button
                    ...
                    class="inline-flex items-center px-3 py-1.5 border border-yellow-300 shadow-sm text-xs font-medium rounded text-yellow-700 bg-white hover:bg-yellow-50"
                  >
                    Revoke
                  </button>
                  <button
      ...
      <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <h3 class="text-lg font-medium text-gray-900 mb-4">Create MCP API Key</h3>
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="flex items-center mb-4">
              ...
              <h3 class="ml-4 text-lg font-medium text-gray-900">API Key Created</h3>
            </div>
              ...
              <div>
                ...
                <div class="flex items-center space-x-2">
                  <input
                    ...
                    :value="createdApiKey"
                    readonly
                    class="flex-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 bg-gray-50 font-mono text-sm focus:outline-none"
                  />
                  <button
                    @click="showApiKey = !showApiKey"
                    class="p-2 border border-gray-300 rounded-md hover:bg-gray-50"
                  >
                    <svg v-if="!showApiKey" class="h-5 w-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <button
                    ...
                    class="p-2 border border-gray-300 rounded-md hover:bg-gray-50"
                  >
                    <svg v-if="!copied" class="h-5 w-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
