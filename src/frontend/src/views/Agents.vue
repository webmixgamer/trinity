        ...
        <div class="flex justify-between items-center mb-8">
          ...
          <div class="flex items-center space-x-4">
            ...
            <select
              ...
              class="block rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2 px-3 bg-white border"
            >
                  ...
                  <router-link :to="`/agents/${agent.name}`" class="flex-1">
                    <div class="flex items-center">
                      ...
                      <div class="ml-4 flex-1">
                        ...
                        <p class="text-sm text-gray-500">Type: {{ agent.type }} | Port: {{ agent.port }}</p>
                        ...
                        <div v-if="hasActivePlan(agent.name)" class="mt-1 text-xs text-gray-500">
                          <span class="inline-flex items-center">
                            <ClipboardDocumentCheckIcon class="h-3 w-3 mr-1 text-purple-500" />
                            {{ getTaskProgress(agent.name) }}
                            <span v-if="getCurrentTask(agent.name)" class="ml-2 truncate max-w-[150px] text-purple-600" :title="getCurrentTask(agent.name)">
