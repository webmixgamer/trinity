<template>
  <div class="space-y-6">
    ...
    <div v-if="loading" class="text-center py-8">
      ...
      <p class="text-gray-500 mt-2">Loading git status...</p>
    </div>

    <!-- Git Not Enabled -->
    <div v-else-if="!gitStatus?.git_enabled" class="text-center py-8 text-gray-500">
      <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
      </svg>
      <p class="mt-2">Git sync not enabled for this agent</p>
      <p class="text-sm text-gray-400 mt-1">Only GitHub-native agents support git sync</p>
    </div>

    <!-- Agent Not Running -->
    <div v-else-if="gitStatus?.agent_running === false" class="text-center py-8 text-gray-500">
      <svg class="mx-auto h-12 w-12 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
      </svg>
      <p class="mt-2">Agent must be running to view git status</p>
      <div v-if="gitStatus?.config" class="mt-4 text-left max-w-md mx-auto bg-gray-50 p-4 rounded-lg">
        <p class="text-sm font-medium text-gray-700">Saved Configuration:</p>
        <p class="text-sm text-gray-500 mt-1">Repo: {{ gitStatus.config.github_repo }}</p>
        <p class="text-sm text-gray-500">Branch: {{ gitStatus.config.working_branch }}</p>
        <p v-if="gitStatus.config.last_sync_at" class="text-sm text-gray-500">
    ...
    <div v-else>
      ...
      <div v-if="gitStatus.changes && gitStatus.changes.length > 0">
        <h3 class="text-sm font-medium text-gray-900 mb-2">Pending Changes ({{ gitStatus.changes_count }})</h3>
        <div class="bg-gray-50 rounded-lg border border-gray-200 overflow-hidden">
          <ul class="divide-y divide-gray-200 max-h-48 overflow-y-auto">
            <li v-for="change in gitStatus.changes" :key="change.path" class="px-4 py-2 flex items-center space-x-3 text-sm">
              ...
              <span class="font-mono text-gray-700 truncate">{{ change.path }}</span>
            </li>
          </ul>
        </div>
      </div>
      ...
      <div v-if="gitStatus.last_commit">
        <h3 class="text-sm font-medium text-gray-900 mb-2">Last Commit</h3>
        <div class="bg-gray-50 rounded-lg border border-gray-200 p-4">
          <div class="flex items-start space-x-3">
            ...
            <div class="flex-1 min-w-0">
              <p class="text-sm font-medium text-gray-900">{{ gitStatus.last_commit.message }}</p>
              <div class="flex items-center space-x-2 mt-1 text-xs text-gray-500">
                <code class="bg-gray-200 px-1.5 py-0.5 rounded">{{ gitStatus.last_commit.short_sha }}</code>
      ...
      <div v-if="gitLog && gitLog.commits && gitLog.commits.length > 1">
        <h3 class="text-sm font-medium text-gray-900 mb-2">Recent Commits</h3>
        <div class="bg-gray-50 rounded-lg border border-gray-200 overflow-hidden">
          <ul class="divide-y divide-gray-200 max-h-64 overflow-y-auto">
            <li v-for="commit in gitLog.commits.slice(1)" :key="commit.sha" class="px-4 py-3">
              <div class="flex items-start space-x-3">
                <code class="flex-shrink-0 bg-gray-200 px-1.5 py-0.5 rounded text-xs">{{ commit.short_sha }}</code>
                <div class="flex-1 min-w-0">
                  <p class="text-sm text-gray-700 truncate">{{ commit.message }}</p>
      ...
      <div v-if="gitStatus.db_config" class="text-xs text-gray-500 border-t pt-4">
        ...
        <p v-if="gitStatus.db_config.last_commit_sha">
          Last synced commit: <code class="bg-gray-100 px-1 rounded">{{ gitStatus.db_config.last_commit_sha.substring(0, 7) }}</code>
        </p>
      </div>
    </div>
  </div>
...
const getChangeStatusClass = (status) => {
  switch (status) {
    case 'M': return 'bg-yellow-100 text-yellow-800'  // Modified
    case 'A': return 'bg-green-100 text-green-800'   // Added
    case 'D': return 'bg-red-100 text-red-800'       // Deleted
    case '?': return 'bg-blue-100 text-blue-800'     // Untracked
    case 'R': return 'bg-purple-100 text-purple-800' // Renamed
