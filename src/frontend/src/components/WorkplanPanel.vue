<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <div>
        <h3 class="text-lg font-medium text-gray-900">Workplan</h3>
        <p class="text-sm text-gray-500" title="This agent's internal task breakdown. Shows how the agent has organized its work into steps.">Explicit planning with task dependencies and progress tracking</p>
      </div>
    ...
    <div v-if="summary" class="grid grid-cols-5 gap-4">
      <div class="bg-white border border-gray-200 rounded-lg p-4">
        ...
        <p class="text-2xl font-semibold text-gray-900">{{ summary.total_plans }}</p>
      </div>
      <div class="bg-white border border-gray-200 rounded-lg p-4">
        <p class="text-xs text-gray-500 mb-1">Active</p>
        <p class="text-2xl font-semibold text-blue-600">{{ summary.active_plans }}</p>
      </div>
      <div class="bg-white border border-gray-200 rounded-lg p-4">
        <p class="text-xs text-gray-500 mb-1">Completed</p>
        <p class="text-2xl font-semibold text-green-600">{{ summary.completed_plans }}</p>
      </div>
      <div class="bg-white border border-gray-200 rounded-lg p-4">
        <p class="text-xs text-gray-500 mb-1">Tasks</p>
        <p class="text-2xl font-semibold text-gray-900">{{ summary.total_tasks }}</p>
      </div>
      <div class="bg-white border border-gray-200 rounded-lg p-4">
        <p class="text-xs text-gray-500 mb-1">Progress</p>
        <div class="flex items-center space-x-2">
          <p class="text-2xl font-semibold text-indigo-600">{{ taskProgressPercent }}%</p>
    ...
    <!-- Current Task Banner -->
    <div v-if="summary?.current_task" class="bg-blue-50 border border-blue-200 rounded-lg p-4">
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-3">
          ...
          <div>
            <p class="text-sm font-medium text-blue-900">Currently Active Task</p>
            <p class="text-sm text-blue-700">
              <span class="font-mono">{{ summary.current_task.task_name }}</span>
              <span class="text-blue-500 ml-2">in {{ summary.current_task.plan_name }}</span>
            </p>
          </div>
        </div>
        <button
    ...
    <div v-if="loading && plans.length === 0" class="text-center py-8">
      ...
      <p class="text-sm text-gray-500 mt-2">Loading workplan...</p>
    </div>

    <!-- Agent Not Running State -->
    <div v-else-if="agentNotRunning" class="text-center py-12 bg-gray-50 rounded-lg">
      <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
      </svg>
      <p class="mt-2 text-gray-500">Agent must be running to view workplan</p>
      <p class="text-sm text-gray-400">Start the agent to access its workplan system</p>
    </div>

    <!-- Empty State -->
    <div v-else-if="plans.length === 0" class="text-center py-12 bg-gray-50 rounded-lg">
      <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
      </svg>
      <p class="mt-2 text-gray-500">No workplan yet</p>
      <p class="text-sm text-gray-400">Workplans are created when the agent uses /workplan commands to decompose complex tasks</p>
    </div>
    ...
    <div v-else class="space-y-3">
      <div
        ...
        class="bg-white border border-gray-200 rounded-lg p-4 hover:border-indigo-300 cursor-pointer transition-colors"
      >
        <div class="flex items-start justify-between">
          ...
          <div class="text-right">
            <p class="text-xs text-gray-400">{{ formatRelativeTime(plan.updated || plan.created) }}</p>
            <div class="flex items-center justify-end space-x-1 mt-1">
              ...
              <span class="text-xs text-gray-500 w-8 text-right">{{ plan.progress_percent }}%</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="selectedPlan" class="fixed inset-0 z-50 overflow-y-auto">
      <div class="flex items-center justify-center min-h-screen px-4">
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75" @click="selectedPlan = null"></div>
        <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full relative z-10 max-h-[85vh] overflow-hidden flex flex-col">
          ...
          <div class="p-4 border-b border-gray-200 flex justify-between items-start">
            <div>
              <div class="flex items-center space-x-2">
                <h3 class="text-lg font-medium text-gray-900">{{ selectedPlan.name }}</h3>
                <span :class="getStatusBadgeClass(selectedPlan.status)">
                  {{ selectedPlan.status }}
                </span>
              </div>
              <p v-if="selectedPlan.description" class="text-sm text-gray-500 mt-1">{{ selectedPlan.description }}</p>
            ...
            <button @click="selectedPlan = null" class="text-gray-400 hover:text-gray-600">
          ...
          <div class="px-4 py-3 bg-gray-50 border-b border-gray-200">
            <div class="flex items-center justify-between mb-2">
              ...
              <span class="text-sm text-gray-500">{{ completedTaskCount }}/{{ selectedPlan.tasks?.length || 0 }} tasks</span>
            </div>
                    ...
                    <div class="flex-1">
                      ...
                      <div v-if="task.dependencies?.length" class="mt-2 flex items-center space-x-1">
                        ...
                        <span
                          ...
                          :key="dep"
                          class="text-xs font-mono px-1.5 py-0.5 rounded bg-gray-100 text-gray-600"
                        >
                          {{ dep }}
                        </span>
                      </div>
                      ...
                      <!-- Result -->
                      <div v-if="task.result" class="mt-2 bg-gray-50 rounded p-2 text-xs font-mono text-gray-600 max-h-24 overflow-y-auto">
                        {{ task.result }}
                      </div>
                    </div>
                  </div>
...
function getTaskBorderClass(status) {
  switch (status) {
    ...
    case 'active': return 'border-blue-300 bg-blue-50/50'
    case 'failed': return 'border-red-200 bg-red-50/50'
    case 'blocked': return 'border-yellow-200 bg-yellow-50/50'
