...
async def get_container_stats():
    ...
    try:
        ...
        containers_stats: List[Dict[str, Any]] = []
        ...
        for agent in running_agents:
            try:
                ...
                container = docker_client.containers.get(f"agent-{agent.name}")

                # Get stats (one-shot, non-blocking)
                ...
                if system_delta > 0 and cpu_delta > 0:
                    # Get number of CPUs for accurate percentage
                ...
                # Subtract cache from memory usage for more accurate metric
        ...
        return {
            ...
            "containers": sorted(containers_stats, key=lambda x: x.get('cpu', 0), reverse=True),
            "timestamp": datetime.utcnow().isoformat() + "Z"
        }
