...
| T4.2 | Agent timeout | ⚠️ Partial | Timeout detected but step status stuck |
...
- **Timeout triggered**: ✅ Error shows `code: 'TIMEOUT'`, `failed_at` timestamp
- **Step status updated**: ❌ Step remains `running` instead of `failed`
...
| T5.5 | Recovery after backend restart | ⚠️ Skip | Requires manual restart |
...
- **Status**: ⚠️ Known limitation - Documented for future implementation
