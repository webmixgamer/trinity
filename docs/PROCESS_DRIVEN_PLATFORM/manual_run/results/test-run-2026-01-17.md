...
| ID | Test | Status | Notes |
|----|------|--------|-------|
| T3.1 | Approval - approved | ✅ Pass | Full approval flow works |
| T3.2 | Approval - rejected | ✅ Pass | Rejection with on_error:skip_step |
| T3.3 | Approval timeout | ⚠️ Skip | Timeout enforcement not implemented |
| T3.4 | Approval with artifacts | ✅ Pass | Context in description |
...
- **Status**: ✅ Pass
- **Paused at approval**: ✅ Status showed `paused` + step `waiting_approval`
- **Continued after approve**: ✅ POST `/api/approvals/{id}/approve` worked
- **Approval metadata available**: ✅ Comment recorded: "Looks good!"
...
- **Status**: ✅ Pass
- **Rejection submitted**: ✅ POST `/api/approvals/{id}/reject` worked
- **Step marked SKIPPED**: ✅ (due to `on_error: skip_step`)
- **Process completed**: ✅ Status COMPLETED (not FAILED)
...
- **Timeout triggered**: ❌ Not enforced
...
| ID | Test | Status | Notes |
|----|------|--------|-------|
| T4.1 | Agent returns error | ✅ Pass | AGENT_UNAVAILABLE error triggers failure |
| T4.2 | Agent timeout | ✅ Pass | Fixed: TIMEOUT now non-retryable |
| T4.3 | Retry policy works | ✅ Pass | Execution fails after retries |
| T4.4 | Skip on error | ✅ Pass | on_error:skip_step works |
| T4.5 | Cancel running execution | ✅ Pass | Cancel API works |

### T4.1 - Agent Error
- **Process File**: `processes/tier4/t4.1-agent-error.yaml`
- **Execution ID**: `ba10e108-47fc-467b-8ece-fbbb57c3f101`
- **Status**: ✅ Pass
- **Error propagated to step**: ✅ `AGENT_UNAVAILABLE`
...
- **Execution ID**: `221423eb-9910-4b00-8ba8-4f137df758f8` (re-test after fix)
- **Status**: ✅ Pass (Fixed)
- **Timeout triggered**: ✅ Error shows `code: 'TIMEOUT'`
- **Step status updated**: ✅ Step now transitions to `failed` immediately
- **Notes**: **FIX APPLIED**: Added `TIMEOUT` to `non_retryable_errors` in `execution_engine.py`. Timeouts now cause immediate step failure without retry attempts.

### T4.3 - Retry Policy
- **Process File**: `processes/tier4/t4.3-retry-policy.yaml`
- **Execution ID**: `e92f4288-4f55-485a-8b3b-171ed64be302`
- **Status**: ✅ Pass
- **Step eventually fails**: ✅
- **Execution fails**: ✅
...
- **Status**: ✅ Pass
- **Failed step marked SKIPPED**: ✅ `optional-step: skipped`
- **Process completed**: ✅ Status `completed`
...
- **Status**: ✅ Pass
- **Cancel API worked**: ✅ `POST /api/executions/{id}/cancel` returns 200
...
| ID | Test | Status | Notes |
|----|------|--------|-------|
| T5.1 | 10-step sequential | ✅ Pass | 10 steps in ~60s |
| T5.2 | Diamond pattern | ✅ Pass | Double diamond with parallel branches |
| T5.3 | Deeply nested expressions | ✅ Pass | 3 steps, nested data |
| T5.4 | Concurrent executions (3x) | ✅ Pass | 3 concurrent running |
| T5.5 | Recovery after backend restart | ✅ Pass | Recovery service resumed execution |

### T5.1 - Ten Step Chain
- **Process File**: `processes/tier5/t5.1-ten-step-chain.yaml`
- **Execution ID**: `17daf3e7-da83-463a-be22-cf8d9d733679`
- **Status**: ✅ Pass
- **All 10 steps completed**: ✅ (step-01 through step-10)
...
- **Parallel branches verified**: ✅ (b1||b2, d1||d2)
...
- **Status**: ✅ Pass
- **3 concurrent started**: ✅ All 3 running simultaneously
...
- **Execution ID**: `4d5b8b68-e0e7-4028-b658-594cfea40768`
- **Status**: ✅ Pass
- **Recovery service ran**: ✅ Found 2 executions, resumed 1, retried 1
- **Execution continued after restart**: ✅ slow-step continued running
- **No recovery errors**: ✅
- **Notes**: Backend restarted while slow-step was running. Recovery service on startup correctly identified and resumed the in-progress execution. Recovery report: `{resumed: 1, retried: 1, failed: 0, errors: 0}`
...
- **Test**: T3.3
- **Severity**: Low
- **Root Cause**: Paused executions aren't re-polled; deadline check added but only runs when handler is invoked
- **Current Behavior**: Approval stays `waiting_approval` indefinitely even after deadline
- **Fix Applied**: Added deadline check in `human_approval.py` handler (checks on re-execution)
- **Fix Needed**: Implement scheduler/background job to periodically check expired approvals and resume executions
- **Status**: ⚠️ Known limitation - Needs background scheduler for full enforcement
...
### Issue #7: Step timeout doesn't update step status (RESOLVED)
- **Test**: T4.2
- **Severity**: High
- **Root Cause**: Default retry policy (3 attempts) kept step in `running` during retry phase
- **Fix**: Added `TIMEOUT` to `non_retryable_errors` in `execution_engine.py`
- **Result**: Timeout now causes immediate step failure, no retries
- **Status**: ✅ RESOLVED
...
- **Status**: 🔴 BUG - Needs investigation and fix
...
- **Human Approval UI (Live Test)**:
  ...
  3. No browser console errors during approval flow ✅
  4. Skipped steps don't show WHY they were skipped (condition not met? on_error? rejection?)
  5. **Page doesn't auto-refresh when approval step becomes active - must manually refresh** ⚠️ (confirmed in I1, I2, I3)
